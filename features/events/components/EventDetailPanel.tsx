      ...
      <div className="flex flex-col h-full animate-[fadeIn_0.5s_ease-out]">
        ...
        <div className="p-6 border-b border-gray-100 bg-white">
          <div className="flex justify-between items-start mb-3">
            <h1 className="font-headline text-3xl font-black text-gray-900 leading-tight">
          ...
          <div className="flex flex-wrap gap-y-2 gap-x-6 mt-4 text-sm font-body text-gray-600">
            {event.eventDate && (
        ...
        <div className="p-6 bg-gray-50 flex-1 flex flex-col">
          <div className="flex items-center justify-between mb-6">
            ...
            {!isCreatingTask && (
              <button
                ...
                className="px-3 py-1.5 bg-white border border-gray-200 hover:border-gray-300 rounded-md text-sm font-bold text-gray-700 transition-colors flex items-center gap-1.5 shadow-sm"
              >
          ...
          {isCreatingTask && (
            <form onSubmit={handleCreateTask} className="mb-6 bg-white p-4 rounded-xl border border-blue-200 shadow-[0_4px_12px_-4px_rgba(0,0,0,0.1)]">
              <div className="mb-3">
                <input
                  type="text"
                  name="title"
                  placeholder="¿Qué se necesita hacer?"
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-body text-sm"
                  autoFocus
                  required
                />
              </div>
              <div className="mb-3">
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-body text-sm min-h-[60px]"
                ></textarea>
              </div>
              
              <div className="mb-4 bg-gray-50 p-3 rounded-lg border border-gray-100">
                <label className="block text-xs font-bold text-gray-700 mb-2">Asignar Voluntarios (Opcional)</label>
                <div className="max-h-32 overflow-y-auto space-y-1 scrollbar-thin">
                  {volunteers.map(vol => (
                    <label key={vol.id} className="flex items-center gap-2 p-1.5 hover:bg-gray-100 rounded cursor-pointer">
                      <input 
                        type="checkbox" 
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-700">{vol.user?.name}</span>
                      </div>
                    </label>
                  ))}
                  {volunteers.length === 0 && (
                    <p className="text-xs text-gray-500 italic">No hay voluntarios disponibles.</p>
                  )}
                </div>
              </div>

              <div className="flex items-center justify-between">
                <select name="priority" className="text-xs border-gray-200 rounded-md py-1.5 pl-2 pr-8 focus:ring-blue-500">
          ...
          <div className="flex flex-col gap-2">
            {loadingTasks ? (
              ...
              <div className="text-center py-10 bg-white border border-gray-200 border-dashed rounded-xl">
                ...
                <p className="font-body text-sm font-medium text-gray-500">Aún no hay tareas</p>
                <button 
                  onClick={() => setIsCreatingTask(true)}
              ...
              tasks.map((task) => {
                ...
                return (
                  <div key={task.id} className="bg-white p-3 rounded-lg border border-gray-200 hover:border-blue-300 shadow-sm transition-all flex items-start gap-3 group cursor-pointer" onClick={() => setSelectedTaskId(task.id)}>
                    ...
                    <div className="flex-1 min-w-0 flex justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <p className={`font-headline text-sm font-bold truncate ${task.taskStatus === 'COMPLETED' ? 'line-through text-gray-400' : 'text-gray-900'}`}>
                          {task.title}
                        </p>
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        ...
                        <div className="relative" onClick={(e) => e.stopPropagation()}>
                          <button
                            ...
                            className="flex items-center justify-between min-w-[100px] px-2 py-1 rounded border hover:bg-gray-50 transition-colors text-xs font-bold"
                          ...
                          {activeTaskDropdown === task.id && (
                            <div className="absolute right-0 top-full mt-1 w-40 bg-white rounded-lg shadow-xl border border-gray-100 z-20 py-1">
                              {TASK_STATUSES.map(s => (
                                <button
                                  key={s.id}
                            ...
                            <>
                              <div className="flex -space-x-1.5">
                                {displayVolunteers.map((vol: any) => (
                                  <div key={vol.id} className="w-6 h-6 rounded-full border-2 border-white bg-blue-100 flex items-center justify-center overflow-hidden shrink-0" title={vol.user?.name}>
                                    {vol.user?.imageUrl ? (
                              ...
                              {extraCount > 0 && (
                                <div className="w-6 h-6 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center shrink-0">
                                  <span className="text-[9px] font-bold text-gray-600">+{extraCount}</span>
                                </div>
                              )}
                            </>
                          ) : (
